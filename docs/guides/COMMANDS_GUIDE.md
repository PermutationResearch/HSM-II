...
# 3. Daemon mode (24/7)
./tools/scripts/macos/run-personal-agent.command --daemon --discord
```
...
```bash
./tools/scripts/macos/run-hyper-stigmergy-II.command  # Unchanged
./tools/scripts/macos/open-hypergraphd.command         # Unchanged
```

### New option available:
```bash
./tools/scripts/macos/run-personal-agent.command       # NEW - Try it!
```

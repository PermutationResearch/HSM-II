...
import {
  ...
  strictAllowedHosts,
  strictAllowedTools,
  strictSecurityEnabled,
  strictUntrustedContentInstruction,
} from "@/app/lib/strict-security";
...
export function operatorChatShouldRouteWorker(params: {
  ...
  // Optional worker-default mode for non-trivial turns.
  // Disabled by default to keep conversational questions fast.
  if (process.env.HSM_OPERATOR_CHAT_NON_TRIVIAL_DEFAULT_WORKER === "1") {
    const trivial =
...
export type SentinelReputationCommand = {
  query: string;
  raw: string;
};

export function parseSentinelReputationCommand(text: string): SentinelReputationCommand | null {
  const t = text.trim();
  if (!t) return null;
  const slash = /^\/sentinel-reputation\s+(.+)$/i.exec(t);
  if (slash?.[1]?.trim()) return { query: slash[1].trim(), raw: t };
  const plain = /^sentinel\s+reputation\s+(.+)$/i.exec(t);
  if (plain?.[1]?.trim()) return { query: plain[1].trim(), raw: t };
  return null;
}

function sentinelReputationBaseUrl(): string {
  return (process.env.HSM_SENTINEL_REPUTATION_URL ?? "http://127.0.0.1:8091").replace(/\/+$/, "");
}

export async function querySentinelReputation(params: {
  query: string;
  timeoutMs?: number;
  ...
  try {
    ...
    const url = `${sentinelReputationBaseUrl()}/reputation?query=${q}`;
    if (strictSecurityEnabled()) {
      const deny = assertUrlAllowed(url);
    ...
    if (!res.ok) {
      ...
      return { ok: false, status: res.status, error: errObj.error ?? `Sentinel API ${res.status}`, data };
    }
...
export async function buildSystemPrompt(
  ...
  if (!companyId) {
    ...
    if (strictSecurityEnabled()) base.push(strictUntrustedContentInstruction());
    return base.join("\n");
  }
...
export function deriveToolExecutionPolicy(
  ...
  const out: ToolExecutionPolicySnapshot = {
    sandbox_mode,
  ...
  if (!strictSecurityEnabled()) return out;
  return {
    sandbox_mode: "workspace_write",
    allowed_tools: strictAllowedTools(),
    network_boundary: {
      allowed_hosts: strictAllowedHosts(),
      block_network_for_bash: true,
    },
    exfiltration: {
      enabled: true,
      max_output_chars: Math.min(out.exfiltration.max_output_chars, 4000),
    },
  };
}
...
export async function dispatchSkillToWorker(params: {
  ...
  while (Date.now() < endAt) {
    ...
    const observedWorker =
      observedFromTask || highValueFromRuntime || highValueFromSummary.size > 0 || highValueFromMeta.size > 0;
    ...
    if (taskRunStatus === "success" || taskRunStatus === "error") {
      ...
      // Skill-specific acceptance gate: validate-delivery must emit PASS / FAIL / BLOCKED.
      if (skillSlug === "validate-delivery" && taskRunStatus === "success") {
        const combinedText = [latestSummary ?? "", task?.run?.log_tail ?? ""].join("\n");
        const hasVerdict = /\b(PASS|FAIL|BLOCKED)\b/.test(combinedText);
        if (!hasVerdict) {
          const summary =
            "validate-delivery finished without a PASS/FAIL/BLOCKED verdict; run rejected. " +
            "The worker must inspect build/test/requirements evidence and state an explicit verdict.";
          await fetch(`${UPSTREAM}/api/company/companies/${companyId}/agent-runs/${runId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
          ...
          return { ok: false, error: summary, httpStatus: 409, runId };
        }
      }
      await fetch(`${UPSTREAM}/api/company/companies/${companyId}/agent-runs/${runId}`, {
        method: "PATCH",

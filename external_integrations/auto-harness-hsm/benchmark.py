...
if __name__ == "__main__":
    ...
    cfg = _load_config()
    backend = cfg.get("benchmark_backend", "tau_bench")
    ...
    parser.add_argument("--domain", default=cfg.get("domain", "retail"))
    args = parser.parse_args()

    if backend == "yc_hsm":
        from ychsm_benchmark_runner import YcHsmBenchRunner

        runner = YcHsmBenchRunner(split=args.split)
    ...
    for task_id, reward in sorted(results.items(), key=lambda x: (str(x[0]).startswith("_"), x[0])):

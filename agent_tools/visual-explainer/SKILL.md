...
**CRITICAL: NEVER generate ASCII art, emoji diagrams, or text-based representations.** The output MUST be valid HTML with CSS styling. If you cannot generate proper HTML, do not use this skill.
...
**Text MUST be readable.** This is critical - white text on white background is a failure. Always ensure sufficient contrast:
- Light theme: Dark text (`#1a1a1a` to `#292017`) on light background (`#faf7f5` to `#ffffff`)
- Dark theme: Light text (`#ede5dd` to `#f0f0f0`) on dark background (`#1a1412` to `#231d1a`)
- Test your colors: if you can't read it, users can't either. The `--text` variable MUST contrast with `--bg`.
...
- **Text readability**: Can you read every piece of text? White-on-white or black-on-black is an automatic failure. Check both light and dark modes.
...
- **NOT ASCII art**: The output MUST be HTML with CSS, NOT text diagrams with emoji bullets.

...
impl FlagStore {
    ...
    pub async fn evaluate(&self, flag_key: &str, ctx: &EvaluationContext) -> bool {
        ...
        let enabled = flags.get(flag_key)
        ...
        *metrics.evaluations.entry(flag_key.to_string()).or_insert(0) += 1;
        if enabled {
            *metrics.enabled_count.entry(flag_key.to_string()).or_insert(0) += 1;
        }
    ...
    pub async fn record_error(&self, flag_key: &str, error: &str) {
        ...
        metrics.errors.entry(flag_key.to_string()).or_default().push(event);
    }
    ...
    pub async fn get_stats(&self, flag_key: &str) -> Option<FlagStats> {
        ...
        Some(FlagStats {
            ...
            error_count: errors,
            error_rate: if evaluations > 0 { errors as f64 / evaluations as f64 } else { 0.0 },
        })
    }
}
...
mod tests {
    ...
    async fn test_percentage_rollout() {
        ...
        assert!(enabled_count >= 40 && enabled_count <= 60, 
            "Expected ~50 enabled, got {}", enabled_count);
    }

...
impl Tool for SystemInfoTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![("detail", "brief or full (default: brief)", false)])
    }
    ...
    async fn execute(&self, _params: Value) -> ToolOutput {
        ...
        ToolOutput::success(summary).with_metadata(info)
    }
}
...
impl Tool for EnvironmentTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "list",
                "List all environment variables (default: false)",
                false,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        if let Some(var_name) = params.get("get").and_then(|v| v.as_str()) {
            match std::env::var(var_name) {
                ...
                Err(_) => {
                    ToolOutput::error(format!("Environment variable '{}' not found", var_name))
                }
            }
...
impl Tool for ProcessListTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "limit",
                "Maximum number of processes to return (default: 50)",
                false,
            ),
        ])
    }
        ...
        match output {
            ...
            _ => {
                ...
                ToolOutput::success(format!(
                    "Current process PID: {}\n(Full process list unavailable on this system)",
                    pid
                ))
            }
        }
...
impl Tool for FileInfoTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![("path", "Path to file", true)])
    }
            ...
            match tokio::fs::read_dir(&path_obj).await {
                Ok(mut entries) => {
                    ...
                    let output: Vec<String> = items
                        .into_iter()
                        .map(|item| format!("{} {:>10} {}", item.file_type, item.size, item.name))
                        .collect();
...
impl Tool for ReadFileEnhancedTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "offset",
                "Line offset to start from (0-based, default: 0)",
                false,
            ),
            (
                "limit",
...
impl Tool for SearchFilesTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "content_pattern",
                "Text pattern to search in file contents",
                false,
            ),
            (
                "case_sensitive",
                                    ...
                                    let lines: Vec<String> = content
                                        ...
                                        .filter(|(_, line)| {
                                            if case_sensitive {
                                                ...
                                                line.to_lowercase()
                                                    .contains(&content_pat.to_lowercase())
                                            }
                                        })
                                        .map(|(i, line)| {
...
impl Tool for ArchiveCreateTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "files",
                "Files/directories to include (space-separated or array)",
                true,
            ),
            (
                "format",
                "Archive format: zip, tar, tar.gz (auto-detected from extension if not specified)",
                false,
            ),
        ])
    }

...
impl Tool for GitStatusTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![(
            "working_dir",
            "Path to git repository (default: current)",
            false,
        )])
    }
...
impl Tool for GitLogTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "since",
                "Show commits since date (e.g., '1 week ago')",
                false,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            ...
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git log failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitDiffTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            ...
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git diff failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitAddTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "files",
                "Files to add (space-separated, or '.' for all)",
                true,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(vec!["add".to_string(), files.to_string()], working_dir).await {
            ...
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git add failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitCommitTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "no_verify",
                "Bypass pre-commit hooks (default: false)",
                false,
            ),
        ])
    }
...
impl Tool for GitPushTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            ...
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git push failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitPullTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            Ok((stdout, _, 0)) => ToolOutput::success(stdout),
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git pull failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitBranchTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "list",
                "List branches (default: true if no other action)",
                false,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            Ok((stdout, _, 0)) => ToolOutput::success(stdout),
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git branch failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
...
impl Tool for GitCheckoutTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, working_dir).await {
            ...
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git checkout failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match run_git(args, None).await {
            Ok((_, _, 0)) => {
                ...
                let msg = dir
                    .map(|d| format!("Cloned into {}", d))
                    .unwrap_or_else(|| "Cloned repository".to_string());
                ToolOutput::success(msg)
            }
            Ok((_, stderr, code)) => {
                ToolOutput::error(format!("Git clone failed (exit {}): {}", code, stderr))
            }
            Err(e) => ToolOutput::error(format!("Failed to run git: {}", e)),
        }
    }

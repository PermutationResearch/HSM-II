    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match self.safe_eval(expr) {
            Ok(result) => {
                ...
                ToolOutput::success(formatted).with_metadata(serde_json::json!({
                    "result": result,
                    "expression": expr,
                }))
            }
...
impl Tool for UnitConversionTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        if vec!["c", "f", "k"].contains(&from.as_str())
            && vec!["c", "f", "k"].contains(&to.as_str())
        {
...
impl Tool for RandomTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            (
                "operation",
                "number, int, bool, choice, shuffle (default: number)",
                false,
            ),
            ...
            (
                "options",
                "Array of options to choose from (for choice)",
                false,
            ),
...
impl Tool for HashTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "algorithm",
                "Hash algorithm: md5, sha256, simple (default: simple)",
                false,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        ToolOutput::success(hash.clone()).with_metadata(serde_json::json!({
            "hash": hash,
            "algorithm": algorithm,
        }))
    }
...
impl Tool for DateTimeTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            (
                "operation",
                "now, format, parse, add, diff (default: now)",
                false,
            ),
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        match operation {
            "now" => {
                let now = Utc::now();
                ToolOutput::success(now.to_rfc3339()).with_metadata(serde_json::json!({
                    "iso": now.to_rfc3339(),
            ...
            "parse" => {
                ...
                match chrono::NaiveDateTime::parse_from_str(input, format_str) {
                    Ok(dt) => {
                        ...
                        ToolOutput::success(dt_utc.to_rfc3339()).with_metadata(serde_json::json!({

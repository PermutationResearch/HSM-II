...
impl Tool for TextReplaceTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        ToolOutput::success(result).with_metadata(serde_json::json!({
            "replacements": count,
        }))
    }
}
...
impl Tool for TextSplitTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "delimiter",
                "Delimiter to split by (e.g., '\\n' for lines)",
                false,
            ),
            (
                "chunk_size",
                "Split into fixed-size chunks (overrides delimiter)",
                false,
            ),
            (
                "limit",
                "Maximum number of splits (default: unlimited)",
                false,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        ToolOutput::success(format!("Split into {} parts", parts.len())).with_metadata(
            serde_json::json!({
                "count": parts.len(),
                "parts": parts,
            }),
...
impl Tool for TextJoinTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "delimiter",
                "Delimiter to join with (default: newline)",
                false,
            ),
        ])
    }
...
impl Tool for TextCaseTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "to",
                "Target case: upper, lower, title, snake, camel, kebab, pascal",
                true,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        let result = match to {
            ...
            "title" => text
                ...
                .collect::<Vec<_>>()
                .join(" "),
            "snake" => text.to_lowercase().replace(" ", "_").replace("-", "_"),
            "kebab" => text.to_lowercase().replace(" ", "-").replace("_", "-"),
            "camel" => {
...
impl Tool for TextTruncateTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "ellipsis",
                "Add '...' when truncated (default: true)",
                false,
            ),
            (
                "from_end",
                "Truncate from end vs start (default: true)",
                false,
            ),
        ])
    }
...
impl Tool for WordCountTool {
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        ToolOutput::success(summary).with_metadata(stats)
    }
}
...
impl Tool for RegexExtractTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            (
                "pattern",
                "Pattern to extract (e.g., '*.com' for domains, '+*' for phone numbers)",
                true,
            ),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        ToolOutput::success(format!("Found {} matches", matches.len())).with_metadata(
            serde_json::json!({
                "matches": matches,
                "count": matches.len(),
            }),

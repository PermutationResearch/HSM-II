...
impl WebSearchTool {
    ...
    async fn crawl_cloudflare(&self, url: &str, _num_results: usize) -> Result<Vec<SearchResult>> {
        let account_id = self.cf_account_id.as_ref()
            .ok_or_else(|| anyhow!("CF_ACCOUNT_ID not configured"))?;
        let api_token = self.cf_api_token.as_ref()
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow!("Cloudflare crawl initiation failed: {}", error_text));
        }
        
        let crawl_response: CloudflareCrawlResponse = response.json().await?;
        
        if !crawl_response.success {
            return Err(anyhow!("Cloudflare crawl failed: {:?}", crawl_response.errors));
        }
        ...
        for attempt in 0..max_retries {
            ...
            match status.result.status.as_str() {
                ...
                _ => {
                    debug!("Crawl job status: {} (attempt {})", status.result.status, attempt);
                }
            }
        }
    ...
    async fn search_duckduckgo(&self, query: &str, num_results: usize) -> Result<Vec<SearchResult>> {
    ...
    async fn search_brave(&self, query: &str, num_results: usize) -> Result<Vec<SearchResult>> {
        let api_key = self.api_key.as_ref().ok_or_else(|| anyhow!("Brave API key not configured"))?;
        ...
        let results = data.web.results.into_iter()
            .map(|r| SearchResult {
                title: r.title,
                url: r.url,
    ...
    async fn search_serper(&self, query: &str, num_results: usize) -> Result<Vec<SearchResult>> {
        let api_key = self.api_key.as_ref().ok_or_else(|| anyhow!("Serper API key not configured"))?;
        ...
        let results = data.organic.into_iter()
            .map(|r| SearchResult {
                title: r.title,
                url: r.link,
...
impl Tool for WebSearchTool {
    ...
    fn parameters_schema(&self) -> Value {
        object_schema(vec![
            ...
            ("num_results", "Number of results to return (1-10, default 5)", false),
        ])
    }
    ...
    async fn execute(&self, params: Value) -> ToolOutput {
        ...
        let num_results: usize = params.get("num_results")
            .and_then(|v| v.as_u64())
            .unwrap_or(5)
            .clamp(1, 10) as usize;
        ...
        let results = match self.backend {
            ...
            SearchBackend::Brave => {
                match self.search_brave(query, num_results).await {
                    Ok(r) => r,
                    Err(e) => {
            ...
            SearchBackend::Serper => {
                match self.search_serper(query, num_results).await {
                    Ok(r) => r,
                    Err(e) => {
...
fn parse_duckduckgo_html(html: &str, max_results: usize) -> Result<Vec<SearchResult>> {
    ...
    if results.is_empty() {
        ...
        results.push(SearchResult {
            ...
            snippet: format!("Please visit DuckDuckGo directly to search for: {}", 
                html.split("q=").nth(1).unwrap_or("").split('&').next().unwrap_or(""))
                .replace("+", " "),
        });
    }

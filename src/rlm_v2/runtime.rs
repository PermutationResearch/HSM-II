...
use super::trajectory::{
    ContextSummary, IterationSnapshot, SubQueryResultSnapshot, ToolResultSnapshot,
};
use super::{
    build_rlm_system_prompt, generate_query_id, Context, FinalAnswer, LlmBridge, LlmBridgeConfig,
    LlmQuery, RlmAction, RlmError, RlmStats, SandboxConfig, SubQuery, SubQueryResponse, Trajectory,
    TrajectoryStore, DEFAULT_MAX_DEPTH, DEFAULT_MAX_ITERATIONS, DEFAULT_MAX_SUB_QUERIES,
...
impl RlmRuntime {
    ...
    pub async fn execute(
        ...
        for iteration in 0..self.config.max_iterations {
            ...
            let prompt = self.build_iteration_prompt(&query, context, iteration, &system_prompt);
            ...
            let (execution_result, subquery_responses) = match &action {
                ...
                RlmAction::SubQueries { queries } => {
                    ...
                    for resp in &responses {
                        self.working_memory
                            .push(format!("[Query {}]: {}", resp.query_id, resp.result));
                    }
                ...
                RlmAction::RequestContext { request } => {
                    self.working_memory
                        .push(format!("[Context Request]: {}", request));
                    let result = self.executor.execute_action(&action).await?;
                    (result, Vec::new())
                }
            };
    ...
    fn build_iteration_prompt(
        ...
        if context.chunks.len() > 1 {
            ...
            for chunk in &context.chunks {
                ...
                prompt.push_str(&format!(
                    "Chunk {} (lines {}-{}): {}...\n",
                    chunk.index, chunk.start_line, chunk.end_line, preview
                ));
            }
        }
        ...
        match serde_json::from_str::<serde_json::Value>(json_str) {
            Ok(json) => {
                ...
                if let Some(action_type) = json.get("action").and_then(|a| a.as_str()) {
                    match action_type {
                        ...
                        "final" | "Final" | "FINAL" => {
                            if let Some(answer) = json.get("answer") {
                                let final_answer: FinalAnswer =
                                    serde_json::from_value(answer.clone()).unwrap_or_else(|_| {
                                        FinalAnswer {
                                ...
                                Err(RlmError::InvalidAction(
                                    "Missing 'answer' in FINAL action".to_string(),
                                ))
                            }
                        }
                        ...
                        _ => Err(RlmError::InvalidAction(format!(
                            "Unknown action type: {}",
                            action_type
                        ))),
                    }
                    ...
                    if response.contains("FINAL") || response.contains("final answer") {
                        ...
                        Err(RlmError::InvalidAction(
                            "No 'action' field found in response".to_string(),
                        ))
                    }
                }
            }
            Err(e) => {
                ...
                if response.len() < 500 && !response.contains("{") {
                    ...
                    Err(RlmError::InvalidAction(format!(
    ...
    async fn execute_sub_queries(
        ...
        let llm_queries: Vec<LlmQuery> = queries
            .iter()
            .filter_map(|sq| {
                context.get_chunk(sq.chunk_index).map(|chunk| LlmQuery {
                    id: sq.id.clone(),
        ...
        let responses: Vec<SubQueryResponse> = results.into_iter().filter_map(|r| r.ok()).collect();
    ...
    fn create_iteration_snapshot(
        ...
        let subquery_snapshots = subquery_responses
            .iter()
            .map(|r| {

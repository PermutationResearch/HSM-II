...
#[deprecated(
    note = "Renamed to HsmSqliteStore — this type is SQLite, not the lbug graph database."
)]
...
impl HsmSqliteStore {
    ...
    pub fn insert_belief(&self, belief: &Belief) -> Result<()> {
        let source_json =
            serde_json::to_string(&belief.source).context("Failed to serialize belief source")?;
    ...
    pub fn delete_belief(&self, id: usize) -> Result<()> {
        self.conn
            .execute("DELETE FROM beliefs WHERE id = ?1", params![id as i64])
    ...
    pub fn insert_skill(&self, skill: &Skill) -> Result<()> {
        let level_json =
            serde_json::to_string(&skill.level).context("Failed to serialize skill level")?;
        let source_json =
            serde_json::to_string(&skill.source).context("Failed to serialize skill source")?;
        let status_json =
            serde_json::to_string(&skill.status).context("Failed to serialize skill status")?;
    ...
    pub fn delete_experience(&self, id: usize) -> Result<()> {
        self.conn
            .execute("DELETE FROM experiences WHERE id = ?1", params![id as i64])
        ...
        let rows = stmt
            .query_map([], |row| {
                ...
                Ok(CouncilDecisionRow {
                    ...
                    mode: row.get(2)?,
                    decision: serde_json::from_str(&decision_str)
    ...
    pub fn get_world_snapshot(&self, tick: u64) -> Result<Option<WorldSnapshot>> {
        ...
        let mut rows = stmt
            .query_map(params![tick as i64], |row| {
                Ok(WorldSnapshotRow {
...
fn belief_from_row(r: BeliefRow) -> Result<Belief> {
    let source: BeliefSource =
        serde_json::from_str(&r.source).context("Failed to deserialize belief source")?;
...
fn skill_from_row(r: SkillRow) -> Result<Skill> {
    ...
    let level: SkillLevel =
        serde_json::from_str(&r.level).context("Failed to deserialize skill level")?;
    let source: SkillSource =
        serde_json::from_str(&r.source).context("Failed to deserialize skill source")?;
    let status: SkillStatus =
        serde_json::from_str(&r.status).context("Failed to deserialize skill status")?;
...
fn experience_from_row(r: ExperienceRow) -> Result<Experience> {
    let outcome: ExperienceOutcome =
        serde_json::from_str(&r.outcome).context("Failed to deserialize experience outcome")?;
...
fn prediction_from_row(r: PredictionRow) -> Result<PredictionReport> {
    ...
    let branches: Vec<ScenarioBranch> =
        serde_json::from_str(&r.branches).context("Failed to deserialize prediction branches")?;
...
fn world_snapshot_from_row(r: WorldSnapshotRow) -> Result<WorldSnapshot> {
    ...
    let agents: Vec<AgentSnapshot> =
        serde_json::from_str(&r.agents).context("Failed to deserialize world snapshot agents")?;
...
mod tests {
    ...
    fn test_trust_edge_upsert() {
        ...
        db.upsert_trust_edge("sys-a", "sys-b", &updated_edge)
            .unwrap();

...
mod tests {
    ...
    async fn demo_jw_cold_start() {
        ...
        for i in 1..=5 {
            ...
            println!(
                "Agent {}: evidence={}, weight={:.2}, reliability={:.2}, score={:.3}",
        ...
        assert!(
            nova_score > 0.70,
            "New agent should have viable score due to JW"
        );
    }
    ...
    async fn demo_promise_tracking() {
        ...
        // Pattern detection
        let critical_promises: Vec<_> = social_memory
            .promises
            .values()
            .filter(|p| p.promiser == agent_id && p.sensitivity == DataSensitivity::Secret)
            .collect();

        let critical_broken = critical_promises
            .iter()
        ...
        println!(
            ...
            (critical_promises.len() - critical_broken) as f64 / critical_promises.len() as f64
                * 100.0
        );
    ...
    async fn demo_collaboration_bonus() {
        ...
        social_memory
            .collaborations
            .insert((alice_id, bob_id), collab_stats.clone());
        social_memory
            .collaborations
            .insert((bob_id, alice_id), collab_stats);
        ...
        let alice_bob_collab = social_memory
            .collaborations
            .get(&(alice_id, bob_id))
            .map(|c| c.score())
            .unwrap_or(0.5);
        let alice_charlie_collab = social_memory
            .collaborations
            .get(&(alice_id, charlie_id))
            .map(|c| c.score())
            .unwrap_or(0.5);

        println!("Collaboration Scores:");
        println!(
            "Alice + Bob: {:.3} (5 successful collaborations)",
            alice_bob_collab
        );
        println!(
            "Alice + Charlie: {:.3} (never worked together)",
            alice_charlie_collab
        );
        ...
        let bob_score = 0.88;
        let pair1_combined =
            (alice_score + alice_bob_collab * 0.1) * (bob_score + alice_bob_collab * 0.1);
        ...
        println!(
            "  Individual scores: {:.2} * {:.2} = {:.3}",
            alice_score, bob_score, pair1_combined
        );
        ...
        println!(
            "  Individual scores: {:.2} * {:.2} = {:.3}",
            alice_score, charlie_score, pair2_combined
        );
    ...
    async fn demo_trend_detection() {
        ...
        for i in 0..20 {
            ...
            if i % 5 == 4 {
                println!(
                    "Deliveries {}-{}: avg quality = {:.2}",
                    i - 4,
                    i,
                    scores[(i - 4)..=i].iter().sum::<f64>() / 5.0
                );
            }
        }
    ...
    async fn demo_delegation_decision() {
        ...
        for agent in &agents {
            ...
            let jw = if agent.experience == 0 { 0.85 } else { 0.0 };
        ...
        assert!(
            ...
            "Senior or Steady should beat FastCoder on critical tasks"
        );
    }
}
